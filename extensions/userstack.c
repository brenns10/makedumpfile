...
/* Optional symbols */
INIT_KERN_SYM(fred_rsp0);
INIT_KERN_SYM(__start_init_stack);
INIT_KERN_SYM(__end_init_stack);
INIT_KERN_SYM(__start_init_task);
INIT_KERN_SYM(__end_init_task);
...
#define HAVE_SYM(s) (GET_KERN_SYM(s) != 0)
...
static bool record_task_stack(unsigned long taskp)
{
	...
	unsigned long vma = 0;
	if (HAVE_MEMBER(mm_struct, mm_mt))
		vma = find_vma_mtree(task_mm + MEMBER_OFF(mm_struct, mm_mt), sp);
	else if (HAVE_MEMBER(mm_struct, mm_rb))
...
void entry(void)
{
	...
	if (!GET_KERN_STRUCT_SSIZE(pt_regs)) {
		ERRMSG("error: missing pt_regs size\n");
		return;
	}
	if (!GET_KERN_SYM(init_task)) {
	...
	if (HAVE_MEMBER(thread_union, stack)) {
		THREAD_SIZE = GET_KERN_STRUCT_MEMBER_MSIZE(thread_union, stack);
	} else if (HAVE_SYM(__start_init_stack) &&
		   HAVE_SYM(__end_init_stack) &&
	...
	} else if (HAVE_SYM(__start_init_task) &&
		   HAVE_SYM(__end_init_task) &&
...
int include_page(unsigned long pfn, const void *pcache)
{
